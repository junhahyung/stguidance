...
def parallelize_transformer(pipe):
    ...
    def new_forward(
        ...
        if x.shape[-2] // 2 % get_sequence_parallel_world_size() == 0:
            ...
            raise ValueError(f"x {x.shape} w {get_sequence_parallel_world_size} Cannot split video sequence into ulysses_degree x ring_degree ({get_sequence_parallel_world_size()}) parts evenly")
