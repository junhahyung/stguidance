...
class AsymmetricAttention(nn.Module):
    ...
    def STG_sdpa_attention(self, qkv, x_length=None):
        q, k, v = rearrange(qkv, "(b s) t h d -> t b h s d", b=1)
        seq_len = q.size(-2)
        if x_length is None or x_length > seq_len:
            raise ValueError("x_length must be provided and should be less than or equal to sequence length.")
        
        full_mask = torch.zeros((seq_len, seq_len), device=q.device, dtype=q.dtype)
        full_mask[:x_length, :x_length] = float("-inf")
        full_mask[:x_length, :x_length].fill_diagonal_(0)
        full_mask = full_mask.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, seq_len, seq_len)
    ...
    # def STG_sdpa_attention(self, qkv, x_length=None):
    #     print(f"[DEBUG] qkv shape: {qkv.shape}")
    #     # qkv를 (q, k, v)로 분리한 후 rearrange
    #     q, k, v = rearrange(qkv, "(b s) t h d -> t b h s d", b=1)
        
    #     # perturbed attention 수행 (mask 사용)
    #     seq_len = q.size(-2)
    #     if x_length is None or x_length > seq_len:
    #         raise ValueError("x_length must be provided and should be less than or equal to sequence length.")
        
    #     # 전체 seq_len에 대해 full_mask 생성
    #     full_mask = torch.zeros((seq_len, seq_len), device=q.device, dtype=q.dtype)
    ...
    #     # full_mask를 batch와 num_heads 차원으로 확장
    #     full_mask = full_mask.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, seq_len, seq_len)

    #     # perturbed attention 수행
    #     with torch.autocast("cuda", enabled=False):
    #         with sdpa_attn_ctx():
    ...
    #             # )
    #             out = F.scaled_dot_product_attention(
    #                 q, k, v, attn_mask=full_mask, dropout_p=0.0, is_causal=False
    #             )
